...
export default function DMList() {
  ...
  const createDM = async () => {
    ...
    if (!otherId) return
    const r = await call<{ other_user_id: number }, { conversation_id: number }>(
      '/sns.v1.DMService/GetOrCreateDM',
      { other_user_id: otherId } as any,
      headers
    )
  ...
  return (
    <main>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <input value={other} onChange={e => setOther(e.target.value)} placeholder="相手ユーザーID" />
        <button onClick={createDM} disabled={!other.trim()}>作成/開く</button>
      </div>
      <ul style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {items.map(c => (
          <li key={c.id}>
            <a href={`/dm/${c.id}`}>会話 #{c.id} ({c.memberUserIds.join(', ')})</a>
          </li>
        ))}
      </ul>
    </main>
