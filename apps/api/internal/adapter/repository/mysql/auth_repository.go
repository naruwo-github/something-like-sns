...
func (r *authRepository) FindTenantByHost(ctx context.Context, host string) (*domain.Tenant, error) {
	var t domain.Tenant
	err := r.db.QueryRowContext(ctx, "SELECT t.id, t.slug FROM tenant_domains d JOIN tenants t ON t.id=d.tenant_id WHERE d.domain=?", host).Scan(&t.ID, &t.Slug)
	if err == sql.ErrNoRows {
		if idx := strings.IndexByte(host, '.'); idx > 0 {
			guess := host[:idx]
			err = r.db.QueryRowContext(ctx, "SELECT id, slug FROM tenants WHERE slug=?", guess).Scan(&t.ID, &t.Slug)
		}
	}
	if err != nil {
		return nil, err
...
func (r *authRepository) FindTenantBySlug(ctx context.Context, slug string) (*domain.Tenant, error) {
	var t domain.Tenant
	err := r.db.QueryRowContext(ctx, "SELECT id, slug FROM tenants WHERE slug=?", slug).Scan(&t.ID, &t.Slug)
	if err != nil {
		return nil, err
	}
	return &t, nil
...
func (r *authRepository) FindOrCreateUser(ctx context.Context, authSub, displayName string) (uint64, error) {
	_, err := r.db.ExecContext(ctx, "INSERT INTO users (auth_sub, display_name) VALUES (?, ?) ON DUPLICATE KEY UPDATE display_name=VALUES(display_name)", authSub, displayName)
	if err != nil {
		return 0, err
	}
	var userID uint64
	if err := r.db.QueryRowContext(ctx, "SELECT id FROM users WHERE auth_sub=?", authSub).Scan(&userID); err != nil {
		return 0, err
	}
	return userID, nil
}
...
func (r *authRepository) FindUserMemberships(ctx context.Context, userID uint64) ([]*domain.TenantMembership, error) {
	rows, err := r.db.QueryContext(ctx, "SELECT m.tenant_id, m.role, t.slug FROM tenant_memberships m JOIN tenants t ON t.id=m.tenant_id WHERE m.user_id=? ORDER BY m.tenant_id", userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
