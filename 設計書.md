...
* **サーバ**（必須）: Go 1.22+ / **connect-go**（RPC）/ **echo**（ヘルス/静的）/ **Bun ORM** / MySQL 8.0 / golang-migrate
* **フロント**（必須）: Next.js 14+（App Router）/ React 18 / TanStack Query / Tailwind CSS
...
-- tenant_domains: サブドメイン/カスタムドメイン解決
...
-- users: グローバルユーザー（OIDC連携を想定）
CREATE TABLE users (
  id           BIGINT PRIMARY KEY AUTO_INCREMENT,
  auth_sub     VARCHAR(255) NOT NULL UNIQUE, -- OIDC sub 等
  display_name VARCHAR(64) NOT NULL,
  avatar_url   VARCHAR(512),
  created_at   TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- tenant_memberships: ユーザーのテナント所属とロール
...
-- reactions: Post/Comment への like をポリモーフィックに表現
...
* **Bun ORM**: Select/Insert すべてに `tenant_id` を明示。共通クエリビルダヘルパを用意。
* **エラーハンドリング**: gRPC status codes に準拠（`InvalidArgument/Unauthenticated/PermissionDenied/NotFound/AlreadyExists`）。
* **トランザクション**: 投稿作成＋初回メタ更新など、整合性が必要な操作は `db.RunInTx` を使用。
...
* **テナント解決**: `middleware.ts` で Host から `tenantSlug` を抽出、初回ロードで `ResolveTenant` 呼び出し。
* **データ取得**: TanStack Query（`staleTime: 30s`）。無限スクロールは `useInfiniteQuery`。
* **UI**: `/`=Feed、`/dm`=会話一覧、`/dm/[id]`=メッセージ。Tailwindで簡素に。
* **楽観的更新**: いいねトグルは optimistic→失敗時リバート。
...
migrate:
	golang-migrate -path packages/dbschema/migrations -database "mysql://$${DB_USER}:$${DB_PASS}@tcp($${DB_HOST}:$${DB_PORT})/$${DB_NAME}" up

seed:
	go run apps/api/cmd/seed/main.go

api-dev:
	air -c apps/api/.air.toml   # or go run ./apps/api/cmd/server
